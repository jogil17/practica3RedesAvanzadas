...
class TestApp(unittest.TestCase):
    ...
    def test_cache_connection_failure(self, mock_redis):
        ...
        mock_redis.return_value.ping.side_effect = Exception("Error de conexión")
        cache = get_cache_connection()
    ...
    def test_index(self, mock_get_cache, mock_get_db):
        ...
        mock_get_db.return_value = True  # Simula que la base de datos está conectada
        ...
        with app.test_client() as client:
            ...
            self.assertEqual(json_data["Database"], "Conectado")
            self.assertEqual(json_data["Cache"], "Conectado")

    @patch("app.get_db_connection")
    @patch("app.get_cache_connection")
    def test_health_check(self, mock_get_cache, mock_get_db):
        # Simular que ambos servicios están "conectados"
